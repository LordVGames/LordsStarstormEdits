...
namespace LordsStarstormEdits.SS2Edits.Events
{
    ...
    internal static class NewStormEventText
    {
        ...
        {
            if (!ConfigOptions.NewStormEventText.Value || !Storm.ReworkedStorm.value)
            {
                return;
            }

            MonoDetourHooks.EntityStates.Events.Storm.OnEnter.ILHook(ChangeStormEventText);
            MonoDetourHooks.EntityStates.Events.Storm.FixedUpdate.ILHook(ChangeStormEventEndText);
        }
